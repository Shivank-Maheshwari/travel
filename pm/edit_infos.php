		...
		if($_POST['password']==$_POST['passverif'])
		{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					if($dn['nb']==0 or $_POST['username']==$_SESSION['pmusername'])
					{
						//We edit the user informations
						if(mysql_query('update users set username="'.$username.'", password="'.$password.'", email="'.$email.'", avatar="'.$avatar.'" where id="'.mysql_real_escape_string(
						['pmuserid']).'"'))
						{
							//We dont display the form
							$form = false;
							//We delete the old sessions so the user need to log again
							unset($_SESSION['pmusername'], $_SESSION['pmuserid']);
?>
...
<?php
	...
	if($form)
	{
		...
		else
		{
			...
			$dnn = mysql_fetch_array(mysql_query('select username,password,email,avatar from users where username="'.$_SESSION['pmusername'].'"'));
